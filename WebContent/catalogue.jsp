...
<body>
	...
	<div class="container desc">
		<div class="row">
			...
			<div class="col-lg-6">
				<c:choose>
					<c:when test ="${empty products}"> Il n'y a aucun produit </c:when>
						<c:otherwise>
							<c:forEach items = "${products}" var = "toto">
								<table>
									<tr>
										<td class="nameProduct" data-id="${toto.id}" style="cursor:pointer">
											<font color="blue"><u>
												<c:out value ="${toto.id}"/>
												<c:out value ="${toto.name}" />
											</u></font>
										</td>
									</tr>
								</table>
	...
	<form action="detailsProduct" method="post" id="formId">
