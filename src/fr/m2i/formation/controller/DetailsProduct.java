...
/**
 * Servlet implementation class DetailsServl
 */
@WebServlet("/DetailsServlClasse")
public class DetailsProduct extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("Identification").equals("OK")){
			...
			Product prod = manager.getProduct(id);
			request.setAttribute("produit", prod);
			this.getServletContext().getRequestDispatcher("/detailsProduct.jsp").forward(request, response);
		} else {
